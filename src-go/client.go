...
func generateClientKey(browser Browser, timeout int, disableRedirect bool, proxyURL string) string {
	// 🚀 Senior Fix: إزالة الكوكيز من المفتاح لكي يتم إعادة استخدام نفس العميل (Connection Pool) للطلبات المتزامنة
	configStr := fmt.Sprintf("ja3:%s|ja4r:%s|http2:%s|quic:%s|ua:%s|sni:%s|proxy:%s|timeout:%d|redirect:%t|skipverify:%t|forcehttp1:%t|forcehttp3:%t",
		browser.JA3,
		browser.JA4r,
...
func generateClientKeyser(browser Browser, timeout int, disableRedirect bool, proxyURL string) string {
